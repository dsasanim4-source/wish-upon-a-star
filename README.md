...
### 3. ⏳ 时光胶囊
设定未来的日期封存心愿。愿望会随时间推移逐渐"成熟"，到期后以星门弹窗解锁，读完可直接写一封回信给未来的自己，并在本设备形成回信链。
...
### 8. 🧭 愿望复盘
把原来的装饰性生长树升级为复盘面板：只统计本设备写下或保存过的愿望，展示心愿总数、完成率、待回访、祝福数、进度分布、提醒日历、最近心愿和已实现心愿，帮助用户真正回头管理自己的愿望。
...
管理员面板支持查看公共愿望、漂流瓶、时光胶囊和操作日志。执行最新版 `schema.sql` 后，管理员可以把云端内容“隐藏 / 恢复 / 永久删除”；隐藏内容会被数据库读取策略拦截，不再公开显示，相关操作会写入管理员日志。
